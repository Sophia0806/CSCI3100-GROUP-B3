...
def index(request):
    ...
    data = request.session.get('user_email')
    con_user = models.user.objects.get(email=data)
    #user = models.user.objects.get(email=current_user.email)
    return render(request, 'logweb/index.html', locals())
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        profile.user = con_user
        profile.image = image
        profile.save()
        return render(request, 'logweb/index.html', locals())
    
    return render(request, 'logweb/upload_profile.html', locals())
    ...
    if request.method == "POST":
        ...
        if useremail.strip() and password: 
            ...
            if user.password == password:
                ...
                #request.session['photo'] = str(user.profile.image) 
                # end the test part
                return redirect('/')
            else:
...
def change_password(request):
    if not request.session.get('is_login', None):
        return redirect('/')
    if request.method == 'POST':
        change_form = forms.ChangePwForm(request.POST)
        if change_form.is_valid():
            oldpassword = change_form .cleaned_data.get('oldpassword')
            newpassword = change_form .cleaned_data.get('newpassword')
            ...
            return render(request, 'logweb/change-password.html', locals())
    change_form = forms.ChangePwForm()
    return render(request, 'logweb/change-password.html', locals())
